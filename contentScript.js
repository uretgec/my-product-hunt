...
document.addEventListener("mouseover", function( event ) {
    ...
    let postItemObj = relTarg.closest("div .styles_item__2kQQ5");
    if(postItemObj === null) return;

    // checked data-marked attribute
    if(postItemObj.hasAttribute("data-marked")) return;

    // bordered mouse over div
    // postItemObj.setAttribute("style", "border-left: 3px solid #f64900");

    // find producthunt id
    currentPostItemId = postItemObj.getAttribute("data-test").replace("post-item-","");
    ...
    moreDiv.setAttribute("class", "styles_container__4Ydg1");
    moreDiv.innerHTML = '<div class="styles_loadingblock__4Ydg1"><span class="styles_font__2Nqit styles_orange__3VieU styles_xSmall__1eYHj styles_semiBold__2IC3i styles_lineHeight__2RYYy styles_underline__20yPd styles_uppercase__2YIgd" href="#">Loading more data</span></div>';
    postItemObj.append(moreDiv);
    ...
    .then(function(item) {
        ...
        let itemObj = {
            ...
            hunter: {
                avatarUrl: item.data.post.user.avatarUrl + "?auto=format&auto=compress&codec=mozjpeg&cs=strip&w=30&h=30&fit=crop",
        ...
        if(item.data.post.makers.length > 0) {
            for (let makerid in item.data.post.makers) {
                ...
                itemObj.makers.push({
                    avatarUrl: makerItem.avatarUrl + "?auto=format&auto=compress&codec=mozjpeg&cs=strip&w=30&h=30&fit=crop",
                    name: makerItem.name,
        ...
        if(item.data.post.topics.edges.length > 0) {
            for (let topicid in item.data.post.topics.edges) {
                ...
                itemObj.topics.push({
                    imageUrl: "https://ph-files.imgix.net/" + topicItem.imageUuid + "?auto=format&auto=compress&codec=mozjpeg&cs=strip&w=30&h=30&fit=crop",
                    name: topicItem.name,
                    slug: topicItem.slug,
        ...
        console.log(postItemSlug, item.data.post, itemObj);
    ...
    .then(function(product) {
        ...
        // Let start fill in the extra content
        let extraContent = '';
        ...
        if(product.topics.length > 1) {
            for (let topicid = 1; topicid < product.topics.length; topicid++) {
                ...
                extraContent += '<a class="styles_button__-7l7l styles_smallSize__38yI- styles_subtleVariant__14Msp styles_simpleVariant__1SJ3a styles_button__SCCIK" title="' + topic.name + ' (' + topic.followersCount + ')" href="/topics/' + topic.slug + '" target="_blank"><span class="styles_font__2Nqit styles_xSmall__1eYHj styles_semiBold__2IC3i styles_buttonContainer__2c7tT styles_lineHeight__2RYYy styles_underline__20yPd styles_uppercase__2YIgd">' + topic.name.replace(" ", "-") + '</span></a>';
            }
        } else {
            extraContent += '-';
        }
        ...
        if(product.bookmarks.length > 0) {
            for (let bookmarkid = 0; bookmarkid < product.bookmarks.length; bookmarkid++) {
                ...
                extraContent += '<a class="styles_button__-7l7l styles_smallSize__38yI- styles_subtleVariant__14Msp styles_simpleVariant__1SJ3a styles_button__SCCIK" title="' + bookmark.storeName + ' (' + bookmark.websiteName + ')" href="' + bookmark.redirectPath + '" target="_blank"><span class="styles_font__2Nqit styles_xSmall__1eYHj styles_semiBold__2IC3i styles_buttonContainer__2c7tT styles_lineHeight__2RYYy styles_underline__20yPd styles_uppercase__2YIgd">' + bookmark.storeName.replace(" ", "-") + '</span></a>';
            }
        } else {
            extraContent += '-';
        }
        ...
        moreDiv.innerHTML = extraContent;
        postItemObj.append(moreDiv);
    ...
    // console.log("ifound", postItemObj, relTarg, postItemSlug);
